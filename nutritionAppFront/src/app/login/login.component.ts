...
export class LoginComponent implements OnInit {
  ...
  loginFunction(monForm:NgForm) {
    ...
    try{
      ...
      this.authService.postlogin().subscribe(
        ...
        if (decodeEtat=="True"){
          ...
          if(monForm.value.password === this.defaultPassword){
            ...
            localStorage.setItem('refrechComponent', "1"); // variable pour résoudre le probleme d'ouverture du menu la peremiere fois apres authentification
      ...
      this.toaster.error("Erreur de connexion avec la base de données","Erreur de connexion")
    }
